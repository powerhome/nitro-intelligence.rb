...
- `base_url` is no longer required on an `assistants_config` entry, so `Assistant::ConfigurationError` no longer names it among the missing fields and an entry without one resolves against the shared deployment. That is the point of the change, but it does mean a `base_url` that is absent or misspelled surfaces when a request is made rather than when the assistant is resolved (#98)
